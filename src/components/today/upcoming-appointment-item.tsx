...
import { UpcomingJob } from '@/app/api/upcoming/route';
import Link from 'next/link';

interface UpcomingAppointmentItemProps {
  appointment: UpcomingJob;
  isFirst?: boolean;
  isLast?: boolean;
}
...
function formatReferenceId(id: string): string {
  // Take last 5 chars of UUID and format as reference
...
export function UpcomingAppointmentItem({
  ...
  const timeRange = appointment.scheduled_at ? formatTimeRange(appointment.scheduled_at) : '';
  const dayNumber = appointment.scheduled_at ? formatDayNumber(appointment.scheduled_at) : '';
  const month = appointment.scheduled_at ? formatMonth(appointment.scheduled_at) : '';
  const referenceId = formatReferenceId(appointment.id);
  const serviceLabel = serviceTypeLabels[appointment.service_type] || 'Service';
  const isMultipleVisit = (appointment.customer_total_jobs || 1) > 1;
  ...
  return (
    <div className="flex gap-4">
      ...
      >
        ...
        {/* Service Type + Visit Badge */}
        <div className="flex items-center gap-2 mb-2">
          <span className="font-semibold text-navy-800">{serviceLabel}</span>
          {isMultipleVisit && (
            <span className="px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide bg-violet-100 text-violet-700 rounded-full">
              Multiple
            </span>
          )}
        </div>
