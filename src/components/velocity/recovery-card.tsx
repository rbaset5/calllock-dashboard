 ...
 *
 * For RECOVERY archetype (callback risk / frustrated customers)
...
export function RecoveryCard({
  ...
  const narrative = buildInlineNarrative(data, 'RECOVERY');

  // Build smart tags
  const tags = buildSmartTags(data, 'RECOVERY');

  // Get expanded section label
  const expandedLabel = getExpandedLabel('RECOVERY');
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        {/* Archetype Badge */}
        <span className="px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-300 flex-shrink-0 ml-4">
          Recovery
        </span>
      </div>
        ...
        <div className={cn(
          ...
          >
            <div className="flex items-center text-[#65676B] dark:text-gray-400 text-[14px] font-semibold">
              <div className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center mr-3">
                <History className="text-slate-600 dark:text-slate-400 w-3.5 h-3.5" strokeWidth={2} />
              </div>
              {expandedLabel}
            </div>
            {expanded ? (
          ...
          {expanded && (
            <div className="animate-in fade-in slide-in-from-top-4 duration-300">
              {/* Command Grid */}
              <CommandGrid data={data} archetype="RECOVERY" />

              {/* Call Button */}
              <button
                onClick={handleCall}
                className="mt-4 w-full bg-slate-900 hover:bg-slate-800 text-white rounded-xl px-4 py-3 flex items-center justify-center space-x-2 shadow-sm transition-colors font-semibold"
              >
              ...
              {address && (
                <button
                  ...
                  className="mt-3 w-full flex items-center justify-center gap-2 text-sm text-gray-500 hover:text-slate-700 transition-colors"
                >
