 ...
 *
 * For REVENUE archetype (high-value opportunities)
...
export function RevenueCard({
  ...
  const narrative = buildInlineNarrative(data, 'REVENUE');

  // Build smart tags
  const tags = buildSmartTags(data, 'REVENUE');

  // Get expanded section label
  const expandedLabel = getExpandedLabel('REVENUE');
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        {/* Archetype Badge */}
        <span className="px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 flex-shrink-0 ml-4">
          Revenue
        </span>
      </div>
        ...
        <div className={cn(
          ...
          >
            <div className="flex items-center text-[#65676B] dark:text-gray-400 text-[14px] font-semibold">
              <div className="w-6 h-6 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center mr-3">
                <Wrench className="text-amber-600 dark:text-amber-400 w-3.5 h-3.5" strokeWidth={2} />
              </div>
              {expandedLabel}
            </div>
            {expanded ? (
          ...
          {expanded && (
            <div className="animate-in fade-in slide-in-from-top-4 duration-300">
              {/* Command Grid */}
              <CommandGrid data={data} archetype="REVENUE" />

              {/* Call Button */}
              <button
                onClick={handleCall}
                className="mt-4 w-full bg-amber-500 hover:bg-amber-600 text-white rounded-xl px-4 py-3 flex items-center justify-center space-x-2 shadow-sm transition-colors font-semibold"
              >
              ...
              {address && (
                <button
                  ...
                  className="mt-3 w-full flex items-center justify-center gap-2 text-sm text-gray-500 hover:text-amber-600 transition-colors"
                >
